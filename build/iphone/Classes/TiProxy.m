...
//Rare exceptions to indicate a bug in the _vandaldining code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_vandaldining = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_vandaldining]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.vandaldining.module"
